...
;Line 100 = ifELSE to find HEX command
;Line 346 = Actual HEX command block
;Bottom of file = command string declared
os_command_line:
...
; os_print_space - no registers, just works
; os_print_newline - reset cursor to start of next line
; os_wait_for_key - AX = key pressed

;EAX is full 32-bit value
;AX is lower 16 bits
;AL is lower 8 bits
; AH is bits 8 through 15 (zero based)

;16 bytes per line, space per hex value
;if mod 16 is not even --> either stop printing midline, or pad the rest of the line with zeroes

;quirk: if there is a leading zero - e.g. 0d - for some versions of QEMU it wont print the leading 0, just prints d. thats ok - the idea is to print a space. 

;print certain number of lines, then wait for key. 
...
.loop: 
	...
	mov dx, 0 			; track number of bytes
	.16_loop:
		inc dx

		push ax			; preserve ax from being corrupted by calls & loadsb
	...
	.not_newline:
		...
		pop ax				; retrieve ax to inc
		inc ax

		dec bx
		cmp bx, 0
		je .eof					; break loop if end of file is reached
		
		cmp dx, 16
		jne .16_loop

		push ax					; preserve ax from being corrupted by calls
		call os_print_newline
...
; =====================================================================
	...
	list_string		db 'LS', 0
	hex_string 		db 'HEX', 0 ;command for hex in cli
